...
public class ShowDirectionBus extends FragmentActivity implements
    ...
    private void requestLocationUpdates(FusedLocationProviderClient fusedLocationProviderClient) {
        Task task = fusedLocationProviderClient.getLastLocation();
        task.addOnSuccessListener(location -> {
            Location location1 = (Location) location;
            double lat = location1.getLatitude();
            double lng = location1.getLongitude();
            currentLocation = new CurrentLocation(lat, lng);
            String str = String.valueOf(currentLocation.getLattitiude());
            yeasb = findViewById(R.id.yeasb);
            yeasb.setText(str);
